...
class Surat_umum extends MY_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('Surat_umum_model');
    ...
    function kode()
    {
             $this->db->select('RIGHT(surat_umum.no_surat,2) as no_surat', FALSE);
             $this->db->order_by('no_surat','DESC');    
             $this->db->limit(1);    
             $query = $this->db->get('surat_umum');  //cek dulu apakah ada sudah ada kode di tabel.    
             if($query->num_rows() <> 0){      
                 ...
                 $kodetampil = "RU/"."UMUM-".$tgl.$batas;  //format kode
                 return $kodetampil;  
   }
   ...
   public function hapus_temp4()
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'surat_umum/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'surat_umum/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'surat_umum/index.html';
            $config['first_url'] = base_url() . 'surat_umum/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Surat_umum_model->total_rows($q);
        $surat_umum = $this->Surat_umum_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'surat_umum_data' => $surat_umum,
            'q' => $q,
        ...
        $this->load->view('surat_umum_list', $data);
        $this->load->view('footer');
    }

    public function read($id) 
    {
        $row = $this->Surat_umum_model->get_by_id($id);
        if ($row) {
            $data = array(
		'id' => $row->id,
		...
		'nama_pegawai' => $row->nama_pegawai,
            ...
            $this->load->view('header');
            $this->load->view('surat_umum_read', $data);
            $this->load->view('footer');
        } else {
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('surat_umum/create_action'),
	    'id' => set_value('id'),
        ...
        $data['kode']=$this->kode();
        $this->load->view('header');
        $this->load->view('surat_umum_create', $data);
        $this->load->view('footer');
    }
    ...
    public function create_action() 
    {
		...
		'nama_pegawai' => $this->input->post('nama_pegawai',TRUE),
            ...
            $this->Surat_umum_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('surat_umum/update_action'),
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Surat_umum_model->delete($id);
...
/* End of file surat_umum.php */
/* Location: ./application/controllers/surat_umum.php */
